...
/* `account` is undefined in the /demo prototype, which keeps showing the
...
export default function ProfileScreen({ account }: { account?: OwnProfile | null }) {
  ...
  const [leaderboardCity, setLeaderboardCity] = useState<"innsbruck" | "salzburg">(
    account?.city ?? "innsbruck",
  );
  ...
  return (
    <div className="paper-grain" style={{ background: PAPER }}>
      ...
      {needsSetup && (
        <section className="px-4 pt-5">
          <button
            ...
            className="card-tap flex w-full items-center justify-between gap-3 px-4 py-4 text-left"
      ...
      {showEdit && isLive && <ProfileEditSheet profile={account ?? null} onClose={() => setShowEdit(false)} />}
    </div>
  );
}
